...
export function PropertiesPanel() {
  ...
  if (selectedId.startsWith('frame-')) {
    ...
    return <FrameProperties solide={solide} />;
  }

  const node = nodes.get(selectedId);
  if (node) return <NodeProperties node={node} />;

  const link = links.get(selectedId);
  if (link) return <LinkProperties link={link} />;

  const arc = angleArcs.get(selectedId);
  if (arc) return <ArcProperties arc={arc} />;
...
function FrameProperties({ solide }: { solide: Solide }) {
  const solides = useDiagramStore((s) => s.solides);
    ...
    <div className="properties-panel">
      ...
      <div className="prop-group">
        ...
        <input
          ...
          onChange={(e) => updateSolideFrameLabel(solide.id, e.target.value)}
      ...
      <div className="prop-group">
        ...
        <div className="prop-row">
          <input
            ...
            onChange={(e) => rotateSolideFrame(solide.id, Number(e.target.value))}
            step={15}
          />
      ...
      {otherFrameSolides.length > 0 && (
        <div className="prop-group">
          ...
          {!arcCreating ? (
            <button className="prop-btn" onClick={() => setArcCreating(true)}>
              + Ajouter arc
            </button>
          ) : (
            <div className="prop-arc-targets">
              ...
              {otherFrameSolides.map((s) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    const tx = s.frameX ?? 0;
                    const ty = s.frameY ?? 0;
                    addAngleArc(solide.id, s.id, (fx + tx) / 2, (fy + ty) / 2);
              ...
              <button className="prop-btn prop-btn-cancel" onClick={() => setArcCreating(false)}>
                Annuler
              </button>
            </div>
          )}
...
function ArcProperties({ arc }: { arc: AngleArc }) {
  const solides = useDiagramStore((s) => s.solides);
